...
def generateQuery(scopus_id, depth):
    q = f"""
        MATCH path = (p1:Paper {{scopusId: '{scopus_id}'}})-[:reference*0..${depth}]->(p2)
        UNWIND relationships(path) as rel
        WITH startNode(rel) as a, endNode(rel) as b
        RETURN DISTINCT a.scopusId, b.scopusId, 
    """
    return q
  
def generateSpark(result_set, spark, redis_graph):
    sgr_id = result_set[0][0]
    df = spark.createDataFrame([[sgr_id, '', r.get(sgr_id), getNodeTitle(sgr_id, redis_graph)]], SCHEMA)
    for row in result_set[1::]:
        new_row = [[
            row[1],
            row[0],
            int(r.get(row[1])),
            getNodeTitle(row[1], redis_graph)
        ]]
